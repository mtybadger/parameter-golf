...
import random, re, subprocess, sys, threading, time, uuid, numpy as np, sentencepiece as spm, torch, torch.distributed as dist, torch.nn.functional as F
...
class ShuffledSequenceLoader:
    ...
    def _build_cpu_batch(self, global_tokens, grad_accum_steps):
        """CPU-only work: build a batch in pinned memory, no H2D transfer."""
    ...
    def next_batch(self, global_tokens, grad_accum_steps):
        x, y = self._build_cpu_batch(global_tokens, grad_accum_steps)
        return x.to(self.device, non_blocking=True), y.to(self.device, non_blocking=True)


class PrefetchLoader:
    """Wraps ShuffledSequenceLoader: a background thread pre-builds CPU batches
    so the Python loop + memmap reads overlap with GPU forward/backward.
    The main thread only does the fast H2D copy when a batch is consumed."""

    def __init__(self, loader, h, queue_depth=2):
        self._loader = loader
        self._h = h
        self._queue = Queue(maxsize=queue_depth)
        self._thread = threading.Thread(target=self._worker, daemon=True)
        self._thread.start()

    def _worker(self):
        while True:
            item = self._loader._build_cpu_batch(
                self._h.train_batch_tokens, self._h.grad_accum_steps
            )
    ...
    def next_batch(self, global_tokens, grad_accum_steps):
        ...
        return (
            x_cpu.to(self._loader.device, non_blocking=True),
            y_cpu.to(self._loader.device, non_blocking=True),
        )
...
def train_model(h, device, val_data):
    ...
    train_loader = PrefetchLoader(ShuffledSequenceLoader(h, device), h)
    max_wallclock_ms = (
