...
class Hyperparameters():
    ...
    hadamard_block_dim = int(os.environ.get("HADAMARD_BLOCK_DIM", 8))
    hadamard_min_numel = int(os.environ.get("HADAMARD_MIN_NUMEL", 65536))
...
def _blockify_weight(t: Tensor, block_dim: int) -> tuple[Tensor, tuple[int, ...]]:
    ...
    if t.shape[1] % block_dim != 0:
        raise ValueError(
            f"Tensor shape {tuple(t.shape)} is not compatible with HADAMARD_BLOCK_DIM={block_dim}"
        )
...
def _should_hadamard_preprocess(name: str, t: Tensor, h: Hyperparameters) -> bool:
    return (
        t.is_floating_point()
        and t.ndim == 2
        and t.numel() > h.hadamard_min_numel
        and t.shape[1] % h.hadamard_block_dim == 0
        and not any(pattern in name for pattern in CONTROL_TENSOR_NAME_PATTERNS)
    )


def _dequantize_int8_tensor(q: Tensor, s: Tensor, dtype: torch.dtype) -> Tensor:
    if s.ndim > 0:
...
def quantize_state_dict_int8(
    ...
    if not _is_power_of_two(h.hadamard_block_dim):
        raise ValueError(f"HADAMARD_BLOCK_DIM must be a power of 2, got {h.hadamard_block_dim}")
    result: dict[str, Tensor] = {}
    meta: dict[str, object] = {}
    for name, tensor in state_dict.items():
        t = tensor.detach()
        if not t.is_floating_point() or t.numel() <= h.hadamard_min_numel:
        ...
        if any(pattern in name for pattern in CONTROL_TENSOR_NAME_PATTERNS):
            ...
            continue
        if _should_hadamard_preprocess(name, t, h):
            blocks, original_shape = _blockify_weight(t, h.hadamard_block_dim)
            sign_vec = _hadamard_sign_vector(
                name,
                h.hadamard_block_dim,
                device=blocks.device,
                dtype=blocks.dtype,
            )
            q, s = quantize_float_tensor(hadamard_rotate_blocks(blocks, sign_vec))
            result[name + ".q"] = q.cpu().contiguous()
            result[name + ".scale"] = s.cpu().contiguous()
            meta[name] = {
                "type": "int8_hadamard",
                "shape": list(original_shape),
                "block_dim": h.hadamard_block_dim,
            }
            continue
...
def dequantize_state_dict_int8(
    ...
    for name, orig in template_sd.items():
        ...
        q = result[name + ".q"]
        s = result[name + ".scale"]
        if info.get("type") == "int8_hadamard":
            block_dim = int(info["block_dim"])
            blocks = _dequantize_int8_tensor(q, s, orig.dtype).to(device=orig.device)
            sign_vec = _hadamard_sign_vector(name, block_dim, device=blocks.device, dtype=blocks.dtype)
            out[name] = _unblockify_weight(hadamard_unrotate_blocks(blocks, sign_vec), tuple(info["shape"]))
            continue
        if info.get("type") == "int8":
            out[name] = _dequantize_int8_tensor(q, s, orig.dtype)
            continue
        raise ValueError(f"Unsupported compression metadata for {name}: {info!r}")
    return out
...
def serialize(h: Hyperparameters, base_model: torch.nn.Module, code: str) -> int:
    ...
    quant_result, quant_meta = quantize_state_dict_int8(base_model.state_dict(), h)
...
def deserialize(h: Hyperparameters, device: torch.device) -> GPT:
    ...
    deq_state = dequantize_state_dict_int8(quant_state["w"], quant_state["m"], template_sd)
...
def run_evals(
    ...
    timed_eval("final_int8_roundtrip", eval_val, h, device, val_data, compiled_model)
    if h.sliding_window_enabled:
        timed_eval("final_int8_sliding_window", eval_val_sliding, h, device, val_data, eval_model)
